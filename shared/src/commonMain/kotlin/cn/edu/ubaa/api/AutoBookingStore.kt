...
object AutoBookingStore {
    ...
    fun getAllTasks(): List<AutoBookingTask> {
        val taskIds = settings.getStringOrNull(TASK_LIST_KEY)
            ?.split(",")?.map { it.trim() }?.filter { it.isNotEmpty() }
            ?: return emptyList()
        return taskIds.mapNotNull { id ->
            settings.getStringOrNull("$KEY_PREFIX$id")?.let {
                try { json.decodeFromString<AutoBookingTask>(it) } catch (_: Exception) { null }
            }
        }
    }
    ...
    /**
     * 保存任务到本地和 Supabase。
     * Supabase 用于服务端定时执行；本地用于即时 UI 更新和离线访问。
     */
    ...
    private fun getAllTaskIds(): List<String> {
        return settings.getStringOrNull(TASK_LIST_KEY)
            ?.split(",")?.map { it.trim() }?.filter { it.isNotEmpty() }
            ?: emptyList()
    }
}
